...
class ReceivablePayableReport:
	...
	def add_customer_filters(
		...
		# Adding the new filter for i-poc and i-poc name
		if self.filters.get("account_manager"):
			self.qb_selection_filter.append(
				self.ple.party.isin(
					qb.from_(self.customer)
					.select(self.customer.name)
					.where(self.customer.account_manager == self.filters.get("account_manager"))
				)
			)
		if self.filters.get("i_poc_name"):
			self.qb_selection_filter.append(
				self.ple.party.isin(
					qb.from_(self.customer)
					.select(self.customer.name)
					.where(self.customer.i_poc_name == self.filters.get("i_poc_name"))
				)
			)	
	...
	def get_party_details(self, party):
		if party not in self.party_details:
			if self.account_type == "Receivable":
				fields = ["customer_name", "territory", "customer_group", "customer_primary_contact","account_manager","i_poc_name"]
